...
```bash
uv run python .opencode/skills/telegram-collector/scripts/collect_messages.py "@channel_username"
```
...
```bash
# 여러 채팅에서 48시간 메시지 수집
uv run python .opencode/skills/telegram-collector/scripts/collect_messages.py "@group1" "@group2" --hours 48

# 특정 사용자 메시지만 필터링
uv run python .opencode/skills/telegram-collector/scripts/collect_messages.py "@mygroup" --users 123456789 987654321

# 출력 경로 지정
uv run python .opencode/skills/telegram-collector/scripts/collect_messages.py "@channel" -o ./reports/telegram_$(date +%Y%m%d).md
```
